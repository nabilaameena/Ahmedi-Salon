#!/usr/bin/env python3
"""
Sustained stress test at 150 concurrent connections against the LOCAL
Nginx deployment (http://localhost:8088) — same test as stress_test_150.py
but targeting the local stack instead of the remote domain.
"""
import time
import statistics
...
print("=" * 72)
print(f"  SUSTAINED LOAD (LOCAL)  ·  {BASE}")
...
def get_session():
    if not hasattr(tls, "session"):
        ...
        tls.session.headers.update({"User-Agent": "stress-test/1.5-local"})
    return tls.session
